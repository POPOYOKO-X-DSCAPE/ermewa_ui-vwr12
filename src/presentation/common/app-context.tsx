...
import init, { type DocumentStatus } from "../../infrastructure/init";
...
export const AppContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [document, setDocument] = useState<AppContextType["document"]>(null);
      ...
      if (import.meta.env.DEV) {
        if (!cancelled) {
          ...
          setDocument({
            ...
            url: "http://localhost:4000/example.msg",
            type: "msg",
            code: "DCXXXXXXX",
            date: "15/09/2025",
            status: {
              // @ts-ignore explanation <data-mocking>
              label: "offline",
        ...
        const { appProfile, document: serverDocument, error } = await init();
        let url = "";
  ...
  return (
    <AppContext.Provider value={{ loading, appProfile, document, error }}>
      {children}
    </AppContext.Provider>
  );
};
